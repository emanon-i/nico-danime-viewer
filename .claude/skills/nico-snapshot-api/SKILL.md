...
### genre / タグの正規化（dアニメキュレーションタグ）

支店では `genre` は **99.95%「アニメ」一色**でサブジャンルの判別に使えない。代わりに**第1話の `tags` にある
`dアニメ` マーカー付きキュレーションタグ**をサブジャンル的に使う（**接尾 `…_dアニメ` / `…_dアニメストア`・接頭 `dアニメ_…` の両型**。例 `ドラマ/青春_dアニメ`・`dアニメ_音楽系`）。

- **正規化**: マーカー（`_dアニメ(ストア)?$` または `^dアニメ_`）を除去 → `/` 分割 → 各ラベル trim → 大小・全半角統一 → エイリアス吸収。素の `dアニメストア`（配信元）は除外。
- 正規化したキュレーションタグは**通常の `tags` と同じ平面のフラットなタグ**として扱う（別 facet にしない。品質が高いので候補の優先表示はしてよい）。`genre` 欄はフィルタ軸に使わない。
...
- **エンドポイント**: `https://ch.nicovideo.jp/ch2632720/video?rss=2.0`（**支店チャンネル直**＝クライアント側フィルタ不要）。
- **本物の RSS（XML）**。`channel.title` に「dアニメストア ニコニコ支店 チャンネル動画」を含む。`item` = `title` / `link` / `guid` / `pubDate` / `description` / `nicoch:isPremium`。`?page=2…` で過去へ遡れる。
- **snapshot の索引は日次更新**（昼以降の追加は翌日まで載らない）。RSS は **pubDate が日中更新＝毎時級**で、**新着（最新の動画）の高頻度源**。条件付き GET（`If-Modified-Since`/`ETag`）で 304 ならほぼ無負荷。**毎時 page1 ＋ `guid` で増分 dedup**。
- **重要・ID 形式差**: RSS の `link` は `nicovideo.jp/watch/<数値 watch id>`（例 `1781226663`）で、**snapshot の `contentId`（`so…`）と形式が違う**。突合は **redirect もしくは「正規化 title＋pubDate」一致で `contentId` に解決**してから統合。未解決は **RSS-only の新着としてのみ**扱う（誤った id 統合をしない）。
- 公開フィードのため**変更検知アサート必須**（XML パース可・支店名・item 数下限）・低頻度厳守。
