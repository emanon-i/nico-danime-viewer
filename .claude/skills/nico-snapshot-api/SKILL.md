...
- **低頻度アクセス**。連続リクエストは間隔を空ける。snapshot の索引は毎日 **AM5:00** 更新なので、高頻度取得に意味はない。
...
取得できる **16 フィールド**と、sort・filter の可否:

| フィールド | 取得 | sort | filter | 備考 |
|-----------|:---:|:---:|:---:|------|
| `contentId` | ✓ | | **✗** | 動画ID。支店各話は `so…` |
| `title` | ✓ | | | |
...
- **`contentId` / `channelId` は filter 不可**（指定すると 400）。個別話の直接引きはできない。
- **`startTime` の filter は ISO8601＋タイムゾーン必須**: `filters[startTime][gte]=2025-01-01T00:00:00+09:00`。
  TZ を付けないと 400。期間ウィンドウ分割にもこの形式を使う。
...
- [ ] `contentId` / `channelId` を filter に使っていないか（filter 不可）。
