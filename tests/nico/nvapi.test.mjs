...
import { _http, _resetAdaptiveDelay } from '../../scripts/lib/http.mjs'
...
describe('mapNvapiEpisodes (§85 backfill)', () => {
  ...
  it('meta.order があればそれを採用する', () => {
    const eps = mapNvapiEpisodes(1, [
      { meta: { order: 5 }, video: { id: 'so1' } },
      { meta: { order: 6 }, video: { id: 'so2' } },
    ])
    expect(eps.map((e) => e.episodeNo)).toEqual([5, 6])
  })
...
describe('fetchSeriesData 到達性メトリクス（nvapi 慢性失敗の可観測化）', () => {
  beforeEach(() => {
    _resetNvapiStats()
    _resetAdaptiveDelay()
  ...
  it('複数回の呼び出しで ok/failed が累積する', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation(() => {
        call++
        if (call === 2) return Promise.resolve({ status: 404, headers: { get: () => null } })
