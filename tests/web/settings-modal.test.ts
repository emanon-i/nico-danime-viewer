...
describe('F-0035: 設定/情報モーダル', () => {
  ...
  it('2つの表示設定が同じスイッチ表示とアクセシブルな状態を使う', () => {
    const { btn, container } = setup({ showEmpty: true, showUnavailable: false })
    btn.click()

    const toggles = container.querySelectorAll<HTMLInputElement>('.settings-toggle-input')
    expect(toggles).toHaveLength(2)
    expect(toggles[0]?.classList.contains('settings-empty-toggle')).toBe(true)
    expect(toggles[0]?.checked).toBe(true)
    expect(toggles[0]?.getAttribute('role')).toBe('switch')
    expect(toggles[0]?.getAttribute('aria-checked')).toBe('true')
    expect(toggles[1]?.classList.contains('settings-unavail-toggle')).toBe(true)
    expect(toggles[1]?.checked).toBe(false)
    expect(toggles[1]?.getAttribute('role')).toBe('switch')
