...
describe('isStructuralTag（構造的定番タグ・§C）', () => {
  ...
  it('神回系は構造タグにしない＝残す（探したい品質 facet・水着回と同類）', () => {
    expect(isStructuralTag('神回')).toBe(false)
    expect(isStructuralTag('超神回')).toBe(false)
    expect(isStructuralTag('約束された神回')).toBe(false)
...
describe('isHiddenTag / withoutHiddenTagNames（UI 非表示＝クール＋構造）', () => {
  ...
  it('withoutHiddenTagNames は非表示タグを除く（神回・水着回は残す）', () => {
    ...
    expect(got).toEqual(['SF/ファンタジー', '水着回', '神回'])
  })
