...
> 本書は機能契約（spec）。実装はまだ行わない。前提は**実データ検証**（ローカル `data/series/*.json` 88,154 各話・state/Pages live・RSS 生フィード）に基づく。準拠 L2: `data-inventory.md`（description フィールド定義・long-wins）/ `dataflow.md`（源→Store→projection）。
...
- [ ] 例外網羅: 括弧付き役名 / 複合役 `監督・絵コンテ` / 全角 `：` / 半角 `/` / 複数声優 / 社名・英字 / © の `原作／`・`/`・`・` / `【各話概要】…／…` synopsis / `収録曲:` の role誤判定回避
- [ ] 無損失: 抽出結果が `descriptionRaw` の文字を取りこぼさない/捏造しない（property test）
- [ ] 検証: `test_parse_description_*` パス
...
- [ ] nvapi 構造版 vs より長い RSS フラット → 構造版を採用（Dr.STONE so46471524 相当 fixture）
- [ ] nvapi 不在時は RSS を synopsis として採用
- [ ] 同一構造クラス内は従来 long-wins 維持（回帰なし）
- [ ] 検証: `test_description_source_priority` パス
...
- [ ] series JSON に `synopsis/cast/staff/studios/copyright/episodeLinks/descriptionStructured` を出力
- [ ] `descriptionRaw` 後方互換維持・既存 `descriptionFirst` 不変
- [ ] 実行ログにパースメトリクス（成功率・未分類数・fallback数）出力
- [ ] 検証: projection スナップショットテスト／既存 `data/series` schema 後方互換

## Exit Criteria

- [ ] F-0057〜F-0059 の受け入れ条件をすべて満たす
- [ ] ローカル 88k 各話コーパスで分類率 ≥ 99%・未分類段落はログに計上され synopsis に保持（取りこぼしゼロ）
- [ ] 誤検知ゼロの担保: synopsis 本文が cast/staff に誤混入しないことを corpus サンプルで確認
- [ ] `pnpm test` / `typecheck` / `lint` / `build` 通過
