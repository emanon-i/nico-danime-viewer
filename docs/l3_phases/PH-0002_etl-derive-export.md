...
SQLite 上で各話統合・タグ正規化・概要・五十音・クール・フランチャイズ・勢いスコアを導出し、新着 RSS（毎時・ID解決）を取り込み、用途別の静的 JSON（`works/ranking/tags/cours/kana/new/series`）を export する。これで `data/*.json` が一式そろう。dataflow.md 準拠。
