...
**実装**: 作品名判定を検索データの削除条件から外し、全話タグを `series.tags` / `works.tags` に欠落なく保持する。候補辞書だけを別集約し、単独作品だけの作品名タグは候補から除外、2作品以上で共有するタグは候補へ残す。候補は NFKC key で重複排除し、各話タグがシリーズ検索タグへ含まれることを full fetch の公開前に assert する。

**受け入れ条件**: 詳細の作品名タグから元作品を検索できること、タイトル前方一致でも複数作品共有タグは候補に残ること、候補に NFKC 重複がないことをフィクスチャと実 state 監査で確認する。
