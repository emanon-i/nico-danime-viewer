...
| **B7 空シリーズ照合** | 各話0件の正シリーズ（`findEmptyRealSeries`／**isAvailable では絞らない**）が存在 | 各対象を nvapi 再取得（1件1回・1 run 件数上限あり）→ 権威メンバーシップで別の正シリーズから各話を引き取り、未取得話は新規作成。**既存の誤登録データ（第3期 空シリーズ等）を修復**する。空シリーズは稀・治癒後は候補外＝自己沈静化。 |
...
- **B7 の自己再武装**: `findEmptyRealSeries` は「各話0件の正シリーズ」を毎 run 再計算するため、空シリーズが空である限り**毎日候補に入れ直される**（自己再武装）。1 run で治癒できなくても翌日また候補になる＝取りこぼしなく再試行され続ける設計。
- **なぜ isAvailable で絞らないか（真因）**: 誤登録の被害シリーズ（第3期等）は各話を別シリーズ（第1期）に奪われて0件になる。snapshot 出現時の `lastSeenAt` は各話を実際に保持する側（奪った第1期）にしか付かないため、被害シリーズは E7（`applyIsAvailableGrace`）で**必ず `isAvailable=false` に落ちる**。つまり「空 かつ 非available」が被害の典型署名であり、旧実装が `isAvailable !== false` で B7 候補を絞っていたため、被害シリーズが恒久的に対象外＝自己修復が本番で1件も効かなかった（PR #6/#7 で heal が発火しなかった真因）。B7 が各話を奪還すれば同一 run 内の `lastSeenAt` 更新 → E7 で `isAvailable=true` に復帰する（B7→A2→lastSeenAt→E7 の順で1 run 完結）。
