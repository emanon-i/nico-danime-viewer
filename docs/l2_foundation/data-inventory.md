...
| `description`           | `string\|null`               | あらすじ（projection 時に HTML strip。`<br>`/`<p>` を改行化＝PH-0014）。複数源競合は**源優先マージ**: nvapi 構造版（`<br>` 区切り）をフラット RSS より優先、同一構造クラス内のみ long-wins（F-0058） | snapshot / RSS   |
...
| ファイル              | 場所                            | 中身 | 状態                                                                                                                    |
| --------------------- | ------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------- |
| `cours-override.json` | `docs/data/cours-override.json` | —    | **削除済み（`git rm` 済み・ファイルなし）**。PH-0002 で手動クール上書き用として雛形追加されたが未実装の遺物のため除去。 |
