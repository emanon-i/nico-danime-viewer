...
「どのページがあり、各ページに何を出すか（どのデータを使うか）」の叩き台。レイアウト詳細・コンポーネント設計は L3 以降。

| ページ | 出すもの | 使うデータ |
|--------|---------|-----------|
| **トップ** | 再生数ランキング上位 ＋ 新着の軽い一覧 | ランキングJSON（`viewCounter` 上位）、新着JSON（`startTime` 降順） |
| **ランキング** | 再生数ランキング（ページング） | ランキングJSON |
| **新着** | 最新の動画が軽く並ぶ一覧 | 新着JSON（`startTime` 降順。更新頻度は §3） |
| **ジャンル別** | ジャンル一覧 → 該当作品 | タグ/categoryTags 由来のジャンル索引JSON |
| **タグ別** | タグ一覧 → 該当作品 | タグ索引JSON |
| **クール別** | 年×季の一覧 → 該当作品 | クール索引JSON（`startTime`／period／programlist 由来） |
| **作品(シリーズ)詳細** | 各話一覧 ＋ 公式プレイヤーへの deep-link | シリーズJSON（list.json の series id で束ねた各話） |
| **検索** | タイトル/タグでの絞り込み結果 | 全作品インデックスJSON（クライアント側検索） |

- 対応要求: トップ/ランキング=REQ-0002、新着=REQ-0010、ジャンル=REQ-0003、タグ=REQ-0008、クール=REQ-0004、
  作品詳細＋deep-link=REQ-0001/0005、検索=REQ-0001。
