...
v1 は **2 画面**（**トップ＝発見ハブ**／**作品(シリーズ)詳細**）。検索・ランキング・新着・ジャンル/タグ/クール・ぜんぶ見るは
**トップ内で完結**（遷移しない・状態は URL クエリ）。別ページに飛ぶのは作品詳細だけで、そこから同タグ/ジャンルへ回遊する。
各画面の ①ワイヤー（配置＋大きさ・モバイル/デスクトップ）②動線、使うデータ・REQ 対応は別ファイル:

→ **[`screens.md`](screens.md)**（画面設計・アスキーワイヤー）
