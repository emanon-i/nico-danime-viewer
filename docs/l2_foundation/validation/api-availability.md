...
# 検証（D・追加）: 関連シーズン/続編の関係データ

> 実施日 2026-06-15。UA 付き・低頻度・非営利。詳細画面の「2期/続編/前作/関連」導線を、信頼できる関係データで作れるか。

## D-0. nvapi v2 series に関係フィールドはあるか → ✗ 無い

- `v2/series/<id>` の `data` = `detail / totalCount / items` のみ。`detail` = `id, owner, title, description, decoratedDescriptionHtml, thumbnailUrl, isListed, createdAt, updatedAt`。
- raw 全文を `related / sequel / prequel / season / parent / group / work / franchise / prev / next` で走査 → **すべて 0 件**。
- → **続編・前作・関連シリーズを返す関係フィールドは存在しない。**

## D-1. dアニメの「作品」グルーピング ID はあるか → ✗ 確認できず
...
ゆるキャン（支店3シーズン）の各シーズン代表話タグ（実測）:

```
ゆるキャン△         : ゆるキャン△ / ゆるキャン△シリーズ
ゆるキャン△ SEASON２ : ゆるキャン△_SEASON２ / ゆるキャン△ / ゆるキャン△シリーズ
ゆるキャン△ SEASON３ : ゆるキャン△_SEASON３ / ゆるキャン△ / ゆるキャン△シリーズ
```

- 全シーズンが**共通の作品タグ `ゆるキャン△`** と**フランチャイズタグ `ゆるキャン△シリーズ`** を共有。各シーズンは固有タグ（`_SEASON２` 等）も持つ。
- → **正規化した作品タグ／`〜シリーズ` タグでフランチャイズを束ねられる。** タイトルに作品名を含まないスピンオフ（へやキャン 等）も、共有タグがあれば拾える（タイトル語幹マッチでは取りこぼす）。
- **注意**: 全作品が必ず `〜シリーズ` タグを持つ保証はない（タグ依存・表記ゆれ）。作品タグの同定ヒューリスティクスが要る（タイトルと一致する素タグ／`〜シリーズ`／B の `<作品名>_dアニメストア` タグ）。

## D-3. タイトル語幹マッチ → △ 脆い（補助のみ）

- タイトル接頭は概ね一貫（`ゆるキャン△ SEASON２`）だが、季節マーカーが多様（`SEASON２`(全角)/`第2期`/`Ⅱ`/`続`/`無印`/サブタイトルのみ）。
- スピンオフ（`へやキャン`）はタイトルに作品名を含まず**別物扱いになる**＝取りこぼし。略称・記号・全半角の誤マッチ risk も。→ **単独主源には不適、タグ手法の補助**に留める。

## D-4. 手動キュレーション → 最終手段

- 関係表を人手で持てば確実だが**保守コストが乗る**。人気作・主要フランチャイズだけに限定する最小運用なら可（例外補正用）。

## 結論（D）

- **信頼できる関係フィールド／work ID は無い**（nvapi・list.json・programlist いずれも）。
- **低保守で最も信頼できる代替 = D-2 共有フランチャイズタグ**（正規化した作品タグ／`〜シリーズ` タグでグルーピング）。
  **D-3 タイトル語幹**は弱い補助、**D-4 手動表**は例外補正のみ。
- ただしタグ依存ゆえ **100% ではない**（未付与・表記ゆれの取りこぼし）。→ **「2期/続編へ」導線はベストエフォート**（取れた範囲で出す）＋正規化＋少数の手動補正、が現実的。

---

# 検証（更新頻度）: 新着動画を毎時取れる源はあるか → ○ 支店チャンネルRSS

> 実施日 2026-06-15。UA 付き・低頻度・非営利。発見系は日次で十分だが「新着動画を毎時」取りたい。snapshot 索引（日次 AM5:00）がネック。

## 1. snapshot 索引の更新実態 → 日次（当日分は朝バッチ止まり）

- 支店最新（`-startTime`）の max は **2026-06-15 00:30**（当日の最古放送回）止まり。`totalCount=87,327`。
- 当日の昼〜夜にチャンネルへ追加される回（後述 RSS では 06:00〜23:00 に多数）は **snapshot にまだ載っていない** → snapshot は**日次更新で日中の増分が即時反映されない**ことを実測で確認。

## 2. 支店チャンネル（ch2632720）の高頻度な新着経路

| 試行 | 結果 |
|------|------|
| `nvapi …/v1/channels/2632720/videos` | 404 |
| `public-api.ch.nicovideo.jp/v1/channels/ch2632720/videos` | 404 |
| `ch.nicovideo.jp/ch2632720`（チャンネルページ） | 200 |
| **`ch.nicovideo.jp/ch2632720/video?rss=2.0`** | **200・本物の RSS（XML）** |

**チャンネル動画 RSS が決定打**（実測）:

```
channel.title = dアニメストア ニコニコ支店 チャンネル動画‐niconico   ← 支店チャンネル直（絞り込み不要）
lastBuildDate = Mon, 15 Jun 2026 23:00:00 +0900
item 数 = 20（pubDate 降順）
 23:00 とんがり帽子のアトリエ EP11
 18:00 魔入りました！入間くん 第４シリーズ #11
 12:00 クジマ歌えば家ほろろ #10
 08:00 もちにゃん 第115話
 06:00 心臓に復讐を誓って 第12話
 00:30 ゴーストコンサート #11
item 要素: title, link, guid, pubDate, description, nicoch:isPremium
 link  = https://www.nicovideo.jp/watch/1781226663   （watch deep-link。id は数値 watch 形式）
 guid  = tag:nicovideo.jp,2026-06-15:/watch/1781226663
 page=2 も取得可（20件・前日へ遡れる＝バックフィル可能）
```

- **支店スコープ**: チャンネル直の RSS なので**クライアント側フィルタ不要**（全 item が支店）。
- **鮮度**: pubDate が当日 00:30→06:00→08:00→12:00→18:00→23:00 と**日中を通じて更新**。`lastBuildDate` も 23:00。
  snapshot（当日 00:30 止まり）より明確に新しい＝**sub-daily（実質、公開ごと～毎時級）**。
- **取得**: `?page=2,3…` で過去へ遡れる。**毎時 page1 を取得し `guid`/watch id で増分 dedup**すれば毎時新着を低負荷で取れる。

## 3. anime.nicovideo.jp 新着系

- period ページはクール単位（年-季の作品一覧）で**動画の新着源ではない**（更新も低頻度）。新着動画は上記チャンネル RSS が担う。

## 結論（更新頻度）

| 新着源 | 更新頻度 | 支店スコープ | 用途・備考 |
|--------|---------|-------------|-----------|
| snapshot `-startTime` | **日次**（AM5:00 頃） | tagsExact→`channelId==2632720` 絞り | 当日昼以降の増分は反映遅延（実測: 最新 00:30 止まり）。発見系（全件/ランキング/カタログ）はこれで十分 |
| **`ch.nicovideo.jp/ch2632720/video?rss=2.0`** | **sub-daily／毎時可** | **チャンネル直＝絞り不要** | 「最新の動画」(各話)の高頻度源。`link`=watch deep-link、`pubDate`、`page` でバックフィル |
| anime period ページ | クール単位（低頻度） | 支店明示 | クール帰属用。新着動画源ではない |

- **○ 支店の新着動画を毎時（sub-daily）で取れる源がある＝支店チャンネル RSS。** 「新着＝毎時」要求は満たせる。
- **取得方法**: 毎時 RSS page1 を取得 → `guid`/watch id で既知分を除外して増分追加（必要なら page 遡及で初回シード）。**発見系（カタログ/ランキング/全件）は従来どおり snapshot 日次**。
- **注意（要対応）**:
...
# 検証（キュレーションタグの取得・接頭/接尾の両マーカー）

> 実施日 2026-06-16。UA 付き・低頻度・非営利。第1話タグの確実な取得方法と、`dアニメ` マーカーの形を確定。

## 第1話タグは snapshot の contentId 直引きで確実に取れる

- `so36388935`（ソ・ラ・ノ・ヲ・ト 第一話）を `filters[contentId][0]=so36388935`（`q=dアニメストア&targets=tagsExact`・`fields=tags`）で直引き → `total=1`、`channelId=2632720`、tags が一致:
  ```
  アニメ dアニメストア ソ・ラ・ノ・ヲ・ト 第一話 アメイジング・グレイス トランペット
  A-1_Pictures_dアニメ ドラマ/青春_dアニメ 戦争/ミリタリー_dアニメ dアニメ_音楽系 オリジナル(原作なし)アニメ_dアニメ
  ```
- `genre` / `categoryTags` は `アニメ`（無力・再確認）。
- → **第1話タグは snapshot の contentId 直引き（`fields=tags`）で確実に取得できる**（タイトル検索のノイズ・取りこぼしを回避できる）。

## `dアニメ` マーカーは接頭型・接尾型の両方で出る

| 形 | 例 |
|----|----|
| **接尾型** `…_dアニメ` / `…_dアニメストア` | `A-1_Pictures_dアニメ`／`ドラマ/青春_dアニメ`／`戦争/ミリタリー_dアニメ`／`オリジナル(原作なし)アニメ_dアニメ`（ぼっち#8 `so41433905` の `名演奏/名ライブシーン_dアニメ` でも再現） |
| **接頭型** `dアニメ_…` | `dアニメ_音楽系` |
...
- **キュレーションタグの判定**: タグ名に `dアニメ` を含む。ただし**素の `dアニメストア`（配信元マーカー）は除外**。
- **正規化**: `_dアニメ(ストア)?$` または `^dアニメ_` を除去 → `/` で分割 → 各ラベル trim → 大小・全半角統一 → エイリアス吸収。
  - 例: `ドラマ/青春_dアニメ` → `ドラマ`, `青春`／`dアニメ_音楽系` → `音楽系`／`A-1_Pictures_dアニメ` → `A-1_Pictures`（スタジオ/レーベル系）。
  - 実機デモ（`名演奏/名ライブシーン_dアニメ`）→ `名演奏`, `名ライブシーン`。
- **汎用/固有タグ**（`アニメ`／作品名／話数／キャラ名／曲名 等）は正規化せずそのまま残す。

## 結論

- 第1話タグは **snapshot の contentId 直引きで確実取得**。`dアニメ` キュレーションタグは**接頭/接尾の両型** → 抽出規則を両対応に。
- 正規化済みキュレーションタグは品質が高く、**通常タグと同じ平面のタグ**として扱える（別 facet 化は不要）。
