...
/**
 * 構造的な定番タグ（§C）。全話 union（§A）で混ざる「全シリーズに必ず付く構造」だけを
 * UI の候補・チップから隠す（データ＝works.tags/tags.json は保持）。
 * 対象（除外）: **最終回系**（最終回/いい最終回だった/N期最終回/感動の最終回…）と
 * **各話番号系**（第N話/N話/#N/N話目）のみ。
 * 非対象（残す）: 神回系・総集編系・記念回系・水着回・お風呂/温泉・SF/ファンタジー等
 * ＝「探せる内容/品質の facet」は全部残す。
 * 線引き原則: 「全シリーズに必ず付く構造（最終回・各話番号）だけ除外」「それ以外の探せる
 * facet（神回・総集編・記念回・水着回…）は残す」。クール由来タグ(§68)除外は維持。
 */
const STRUCTURAL_TAG = /最終回|^第?\d+話$|^#\d+$|^\d+話目$/u
