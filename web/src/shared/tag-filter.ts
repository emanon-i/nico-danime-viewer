...
/**
 * 構造的な定番タグ（§C）。全話 union（§A）で混ざる「全作品共通の構造的 episode-meta」を
 * UI の候補・チップから隠す（データ＝works.tags/tags.json は保持）。
 * 対象: 最終回系（最終回/いい最終回だった/N期最終回/感動の最終回…）・記念回・総集編系・
 * 各話番号（第N話/N話/#N/N話目）。
 * 非対象（残す）: **神回**・水着回・お風呂/温泉・SF/ファンタジー等＝「探したい内容/品質の facet」。
 * 線引き原則: 「探したい内容/品質（神回・水着回）は残す」「全作品共通の構造（最終回・各話番号）は除く」。
 * 注: DF/クール跨りでは構造タグとジャンルを分離できない（ジャンルが最も普遍）ため、
 * 統計閾値ではなく意味ベースの少数キュレーションで判定する。
 */
const STRUCTURAL_TAG = /最終回|記念回|総集編|^第?\d+話$|^#\d+$|^\d+話目$/u
