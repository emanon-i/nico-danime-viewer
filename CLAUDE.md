...
| `pnpm fetch` | 各源(snapshot/RSS/period/list.json…) → ビルド時 SQLite → `data/*.json` を生成 | 雛形（未実装） |
| `pnpm dev` | `web/` のローカル開発サーバ | 雛形（未実装） |
| `pnpm build` | 静的サイトをビルド → `dist/` | 雛形（未実装） |
| `pnpm test` / `pnpm lint` / `pnpm typecheck` | テスト / Lint / 型チェック | 雛形（未実装） |

実装が進んだら、本表の「状態」を更新し、テスト／型チェックのコマンドもここに追記する。

## コーディング規約

- **言語/構成**: データ取得は Node.js（ESM, `.mjs`）。フロントは静的サイト（**Vite + TypeScript**・**pnpm**）。加工はビルド時 SQLite（→静的 JSON）。
...
- **生成物はコミットしない**: `data/*.json` は `pnpm fetch` の出力。git 管理外（`.gitignore` 済）。中間生成物の SQLite も配信に出さない。
...
**構成方針**: fetch スクリプトで各源を**ビルド時 SQLite に取り込み → 用途別静的 JSON 化 → 静的サイトが読む**（DB は配信に出さない）。
**新着＝チャンネル RSS で毎時／発見系＝snapshot 日次**。詳細は L2 `dataflow.md` / `db-design.md`。
...
| `.claude/skills/` | プロジェクト用 Agent Skill（nico API リファレンス＆ヘルパ） |
...
**データ取得時**: `.claude/skills/` の nico API スキルを参照（支店の取り方・ランキング/各話(nvapi)・RSS新着・タグ正規化・補助源・ToS・CORS）。
